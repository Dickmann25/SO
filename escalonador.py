...
class Escalonador:
    def __init__(self, memoria, recursos, semaforo_processos, semaforo_memoria, semaforo_print):
        #tamanho maximo da fila
        self.capacity = MAX_QUEUE_SIZE
        # Fila de processos de tempo real (RT)
        self.rt_queue = Fila("RT", capacity=self.capacity)
        # Filas de usuario, uma para cada prioridade (1 a 5)
        self.user_queues: Dict[int, Fila] = {p: Fila(f"U{p}", capacity=self.capacity) for p in range(1, 6)}
    ...
    # quando ocorre aging o processo troca de fila
    def reposiciona_prioridade(self, fila):
        temp_fila = self.user_queues[fila].q.copy()
        for proc in temp_fila:
            prioridade_antiga = self.user_queues[fila].q.popleft()
            self.user_queues[proc.current_priority].push(prioridade_antiga)
    ...
    def main(self):
        while True:
            ...
            if proc is not None:
                execucao = self.run_one_slice(proc)
                with self.semaforo_print:
                    print(execucao)

                for fila in self.user_queues:
                    self.reposiciona_prioridade(fila)
