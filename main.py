...
def main():
    ...
    semaforo_print = threading.Semaphore(1)       # controla ordem de print


    # Inicializa os modulos necessarios
    recursos = Recursos()
    memoria = MemoryManager()
    escalanador = Escalonador(memoria, recursos, semaforo_processos, semaforo_memoria, semaforo_print)

    # passa o escalonador para o dispatcher
    dispatcher = Despachador(escalanador, memoria, recursos, process_file, fileops_file, semaforo_processos, semaforo_memoria, semaforo_print)
